...
* Passwords stored in the Entity Store: All sensitive data (local user store passwords, private keys and their passwords, and passwords required to connect to third-party services) can be encrypted in the Entity Store using Password-Based Encryption (PBE) with the Entity Store passphrase. Firstly, a high entropy master key (MK) of size 32 bytes is created using PBKDF2 with inputs of the entity store passphrase, a 16 byte salt generated using a pseudo random number generator (PRNG) and a 100,000 iteration count. MK = PBKDF2(HMAC-SHA512, passphrase, unique salt, iteration count, MK length). This MK is not used for encryption but instead is used as key material input into the generation of a data protection key (DPK). The DPK of size 32 bytes is derived from the MK again using PBKDF2, unique random salt and MK. DPK = PBKDF2(HMAC-SHA512, MK, unique salt, iteration count, DPK length). The number of iterations is 2 for the DPK generation to maintain acceptable performance in the runtime environment. To encrypt plaintext we generate a cipher using AES/GCM encryption, the DPK as the secret key and we store the unique salt with the cipher text.
