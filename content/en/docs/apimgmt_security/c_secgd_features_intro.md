...
* **Passwords stored in the Entity Store**: All sensitive data (local user store passwords, private keys and their passwords, and passwords required to connect to third-party services) can be encrypted in the Entity Store, see [Configure encryption passphrase](/docs/apim_administration/apigtw_admin/general_passphrase/). The process behind the encryption uses Password-Based Encryption (PBE) with the Entity Store passphrase:
